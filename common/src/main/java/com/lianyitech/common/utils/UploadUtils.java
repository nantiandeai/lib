...
public class UploadUtils {
	...
	// 文件保存根目录
	private String uploadRootPath = Global.getUploadRootPath();
	// 文件保存目录相对路径
	private String basePath = Global.UPLOADFILES_BASE_URL;
	// 文件的目录名
	 ...
	 *
	 * @param request
	 * @return infos[0] true代表上传成功、infos[1]返回路径    false代表失败 、infos[1]返回错误消息
	 */
	@SuppressWarnings("unchecked")
	public String[] uploadFile(HttpServletRequest request) {
		String[] infos = new String[2];
		// 验证
		String errorInfo = this.validateFields(request);
		if (!"true".equals(errorInfo)) {
			infos[0] = "false";
			infos[1] = errorInfo;
			return infos;
		}
		...
		if (fiList != null) {
			for (FileItem item : fiList) {
				errorInfo = this.saveFile(item, request);
			}
			if (!"true".equals(errorInfo)) {
				infos[0] = "false";
				infos[1] = errorInfo;
		...
		infos[0] = "true";
		fileUrl=fileUrl.substring(this.uploadRootPath.length(),fileUrl.length());
		infos[1] = fileUrl;
		return infos;
	}
	 ...
	 * @param request
	 */
	public void createDirByFileExt(String fileExt,HttpServletRequest request){
		Iterator iter=extMap.entrySet().iterator();
		while (iter.hasNext()){
			Map.Entry entry = (Map.Entry)iter.next();
			String key = entry.getKey().toString();
			String value = entry.getValue().toString();
			if(value.contains(fileExt)){
				dirName=key;
				break;
			}
		}
	...
	private String validateFields(HttpServletRequest request) {
		...
		// 文件保存目录路径
		savePath = Global.getUploadRootPath() + basePath + "/";
		File saveDirFile = new File(savePath);
		if (!saveDirFile.exists()) {
			saveDirFile.mkdirs();
		}
...
//	@SuppressWarnings("unchecked")
	...
	private String saveFile(FileItem item,HttpServletRequest request) {
		...
		//创建目录
		createDirByFileExt(fileExt,request);
